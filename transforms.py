...
def transforms_state(ts, **kwargs):
    assert isinstance(ts, collections.abc.Sequence)
...
class RandomScale(object):
    def __init__(self, size_range, mode='linear', anchor=None, random_state=np.random):
        assert isinstance(size_range, collections.abc.Sequence) and len(size_range) == 2
...
class Merge(object):
    ...
    def __call__(self, *imgs):
        # ad-hoc 
        if len(imgs) > 1 and not isinstance(imgs[0], collections.abc.Sequence):
            pass
        elif len(imgs) == 1 and isinstance(imgs[0], collections.abc.Sequence):   # unreliable
            imgs = imgs[0]
        elif len(imgs) == 1:
            return imgs[0]
        else:
...
class Compose(object):
    ...
    def __call__(self, *data):
        # ad-hoc 
        if len(data) >= 1 and not isinstance(data[0], collections.abc.Sequence):
            pass
        elif len(data) == 1 and isinstance(data[0], collections.abc.Sequence) and len(data[0]) > 0:   # unreliable
            data = list(data[0])
        else:
            raise Exception('invalid input')

        for t in self.transforms:
            if not isinstance(data, collections.abc.Sequence):   # unreliable
                data = [data]

            if isinstance(t, collections.abc.Sequence):
                if len(t) > 1:
                    assert isinstance(data, collections.abc.Sequence) and len(data) == len(t)
                    ds = []
        ...
        if isinstance(data, collections.abc.Sequence) and len(data) == 1:   # unreliable
            return data[0]
        else:
            return data
