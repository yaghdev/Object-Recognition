...
def train():   
    ...
    while True:
        for input, loc, label in dataloader:
            ...
            if iteration % 10 == 0:
                print("iteration : ", iteration, " loc_loss : ", loc_loss.item(), " conf_loss : ", conf_loss.item(), " loss : ", loss.item())
              
            if iteration % 50 == 0 and iteration != opt.start_iter:
                if not os.path.isdir('weights'):
                        os.mkdir('weights')
                torch.save(model.state_dict(), 'weights/model_{}.pth'.format(iteration))
