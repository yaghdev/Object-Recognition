    ...
    def encode(self, boxes, labels):
        if len(boxes) == 0:
            return (
                ...
                torch.LongTensor(np.zeros(self.anchor_boxes.shape[0], dtype=np.int32)))
        ...
        return torch.FloatTensor(loc.astype(np.float32)), torch.LongTensor(labels.astype(np.int32))
